...
def read_employees(skip: int = 0, limit: int = 10, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
...
def read_employee(employee_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    # Only admin/super admin or the owner can access
    if current_user.role not in ["admin", "super_admin"] and employee.user_id != current_user.id:
        raise HTTPException(status_code=403, detail="Operation not permitted")
    return employee

@router.put("/{employee_id}", response_model=EmployeeOut)
def update_employee(employee_id: int, update: EmployeeUpdate, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
