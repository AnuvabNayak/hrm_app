...
def _rolling_window_start(as_of: datetime) -> datetime:
    try:
        ...
        return as_of - relativedelta(months=12)
    except Exception:
...
def _expiry_from_grant(grant_date: datetime) -> datetime:
    try:
        from dateutil.relativedelta import relativedelta
        return grant_date + relativedelta(months=12)
    except Exception:
...
def get_available_coins(db: Session, employee_id: int, as_of: datetime | None = None) -> dict:
    ...
    for c in coins:
        ...
        if exp <= soon_cutoff:
        # if c.expiry_date <= soon_cutoff:
            expiring[exp] = expiring.get(exp, 0) + c.remaining
    expiring_soon = [{"expiry_date": k, "amount": v} for k, v in sorted(expiring.items(), key=lambda kv: kv)]
    #         expiring[c.expiry_date] = expiring.get(c.expiry_date, 0) + c.remaining
    # expiring_soon = [{"expiry_date": k, "amount": v} for k, v in sorted(expiring.items(), key=lambda kv: kv[0])]
    ...
    recent_txns = [
        {"type": t.type, "amount": t.amount, "occurred_at": _aware_utc(t.occurred_at), "comment": t.comment}
        for t in txns
    ]
